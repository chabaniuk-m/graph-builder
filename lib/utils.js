import {graph, langPack} from "../index.js";
...
export function addNameFromID(id) {
    let nodeEl = document.getElementById(id)
    nodeEl.innerHTML = ""
    nodeEl.innerText = letters[id % 26]
    if (id >= 26) {
        const sub = document.createElement("sub")
        sub.innerText = Math.floor(id / 26)
        nodeEl.appendChild(sub)
    }
}

export function isNameAlreadyPresent(name, id) {
    for (const node of graph.nodes)
        if ("name" in node && node.name === name && node.id != id)
            return true
    return false
}

export function saveName(textarea) {
    let node = graph.nodes.find(node => node.id == textarea.parentNode.id)
    if (isNameAlreadyPresent(textarea.value, textarea.parentNode.id) || textarea.value.length === 0) {
        addNameFromID(parseInt(textarea.parentNode.id))
        if ("name" in node)
            delete node.name
    } else {
        node.name = textarea.value.trim()
        console.log(`Now node name is ${node.name}`)
    }
